...
function get_ingress_type {
        ...
        local currect_vip_type=$(kubectl get svc  -o json| jq '.items[] | select(.metadata.name == "radcom-ingress-nginx-chart-controller").spec.type'|xargs)
...
function get_ingress_vip_by_service {
        ...
        if [ ${currect_vip_type} == "LoadBalancer" ];then
                local current_vip_addr=$(kubectl get svc  -o json| jq '.items[] | select(.metadata.name == "radcom-ingress-nginx-chart-controller").status.loadBalancer.ingress[0].ip'|xargs)
        elif [ ${currect_vip_type} == "ClusterIP" ];then
                local current_vip_addr=$(kubectl get svc  -o json| jq '.items[] | select(.metadata.name == "radcom-ingress-nginx-chart-controller").spec'.${currect_vip_type} |xargs)
        else
...
function wait_for_ace_orch {
        ...
        return_code=$(curl -s -o /dev/null -I -w "%{http_code}" http://${ingess_ip}/orchestrator/isReady)
        while [ ${return_code} -ne 200 ];do
                echo -n "."
                sleep 20
                return_code=$(curl -s -o /dev/null -I -w "%{http_code}" http://${ingess_ip}/orchestrator/isReady)
...
function add_blueprint {     
        ...
        eval curl -f -X POST http://${CONDUCTOR_IP}:8084/orchestrator/v1/addVersionBlueprints -H \"Content-Type: application/json\" -d \'${json_data}\'
}
