...
{{- include "prometheus-targets.promlabels" . }}
{{- end }}
...
{{- define "getAdditionalAnnotations" -}}
    ...
    {{- range $currAnnotationsName, $currAnnotationsValue := $root.Values.ingressAdditionalAnnotationsOptions -}}
        {{ if eq (tpl $currAnnotationsName $root) $AdditionalAnnotationsName }}{{ tpl (toYaml $currAnnotationsValue) $root }}
        {{ end -}}
    {{- end }}    
{{- end }}
...
{{- define "prometheus-targets.promlabels" }}
blueprintdeployment: {{ .Release.Name }}
{{- if .Values.global }}
{{- if .Values.global.site }}
site: {{ .Values.global.site }}
{{- end -}}
{{- if .Values.global.siteType }}
sitetype: {{ .Values.global.siteType }}
{{- end -}}
{{- if .Values.global.blueprinttype }}
